...
class FunctionSignatureTest(unittest.TestCase):
    ...
    def test_infer_signature_without_any_input_annotation(self):
        def func() -> None:
            pass

        self.assertEqual(infer_signature(func=func), Signature(n_in=0, n_out=0))

    def test_infer_signature_with_single_input(self):
        def func(a: int) -> None:
            pass

        self.assertEqual(infer_signature(func=func), Signature(n_in=1, n_out=0))

    def test_infer_signature_with_single_required_input(self):
        def func(a: int, b: int = 0) -> None:
            pass

        self.assertEqual(infer_signature(func=func), Signature(n_in=1, n_out=0))

    def test_infer_signature_with_none_type_input(self):
        def func(a: None) -> None:
            pass

        self.assertEqual(infer_signature(func=func), Signature(n_in=0, n_out=0))

    def test_infer_signature_with_tuple_input(self):
    ...
    def test_infer_signature_with_mixed_input(self):
    ...
    def test_infer_signature_with_variadic_tuple_input(self):
    ...
    def test_infer_signature_without_any_output_annotation(self):
        def func():
            pass

        self.assertEqual(infer_signature(func=func), Signature(n_in=0, n_out=1))

    def test_infer_signature_with_none_output(self):
        def func() -> None:
            pass

        self.assertEqual(infer_signature(func=func), Signature(n_in=0, n_out=0))

    def test_infer_signature_with_single_output(self):
        def func() -> Any:
            pass

        self.assertEqual(infer_signature(func=func), Signature(n_in=0, n_out=1))

    def test_infer_signature_with_tuple_single_output(self):
        def func() -> tuple[Any]:
            pass

        self.assertEqual(infer_signature(func=func), Signature(n_in=0, n_out=1))

    def test_infer_signature_with_tuple_many_outputs(self):
        def func() -> tuple[Any, Any]:
            pass

        self.assertEqual(infer_signature(func=func), Signature(n_in=0, n_out=2))

    def test_infer_signature_with_variadic_tuple_output(self):
        def func() -> tuple[Any, ...]:
            pass
    ...
    def test_infer_signature_with_noreturn_output(self):
        def func() -> NoReturn:
            pass
    ...
    def test_infer_input_shape(self):
...
class FineCallableTest(unittest.TestCase):
    def test_infer_finecallable_signature(self):
    ...
    def test_finecallable_is_callable(self):
        class A(FineCallable):
            pass

        a = A(signature=Signature(n_in=0, n_out=0))
